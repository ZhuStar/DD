...
import { ValidateD5cDocument } from "../../src/character/validate.ts";
import { RuleRegistry } from "../../src/rules/ruleRegistry.ts";
import { CollectRuleRefs } from "../../src/rules/ruleResolver.ts";
...
test("the Elowen fixture parses, validates, normalizes, and resolves cleanly", async () => {
	...
	const _refs = CollectRuleRefs(_document).map((c) => c.ref);
...
test("a missing rule object surfaces as a resolve diagnostic, not a crash", async () => {
	...
	const _refs = CollectRuleRefs(_document)
		.map((c) => c.ref)
